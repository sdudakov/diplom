...
# функция возвращает множество групп пользователя в которых не состоит ни один из его друзей
...
def write_groups_to_json(groups):
    ...
    groups_list = []
    response = requests.get('https://api.vk.com/method/groups.getById', params) #информация о группе
    for response in response.json()['response']:
        if not 'deactivated' in response:
            groups_dict = {'name': response['name'], 'gid': response['id'], 'members_count': response['members_count']}
